...
/**
 * Class StockInventoryControlForm.
 */
class StockInventoryControlForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['#theme'] = ['commerce_simple_stock_inventory_control_form'];
    ...
    // If we have user submitted values, that means this is triggered
    // by form rebuild because of SKU not found.
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($user_submit as $pos => $row) {
      if (!$this->validateSku($row['sku'])) {
        ...
        $this->messenger()->addError($this->t("SKU: @sku doesn't exist.", ['@sku' => $row['sku']]));
      }
    }
    ...
    if (count($invalidSKUPos) > 0) {
      ...
      foreach ($user_submit as $pos => $row) {
        ...
        if ($productVariation->hasField('field_stock')) {
          ...
          $this->messenger()->addMessage($productVariation->getTitle() . ': ' . $productVariation->field_stock->value);
        }
      }
      $this->messenger()->addStatus($this->t('Operation Succeeded!'));
    }
  }
